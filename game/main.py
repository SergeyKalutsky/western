...
class Game:
    def __init__(self, url):
        ...
        self.battle_permissions = get_info(f'http://{self.url}/get_permissions')
        ...
        self.input_field = vkii.TextInput((200, 60), (400, 240), ISCANDER_RED, RED, (100,100,100), font_size=50)

        self.players_count = 2

        self.in_menu = False

        self.table = vkii.Table((1000,700), (50,50,50), ISCANDER_RED, 40, (100,50))
    ...
    def draw(self):
        ...
        if self.input_field.final_text == '':
            ...
            self.screen.blit(self.input_field.rendered_text[0], self.input_field.rendered_text[1])
        ...
        if self.pre_attack_timer.active:
            pre_text = self.font.render(str(int(round(self.pre_attack_timer.time, 0))), False, ISCANDER_RED)
        ...
        if not len(self.battle_permissions) == self.players_count and self.input_field.final_text != '':
            self.screen.blit(self.font.render('Нажмите SPACE для начала игры', False, ISCANDER_RED), (400,400))
    ...
    def shoot(self):
        ...
        print(requests.post(url=f'http://{self.url}/post_score', json={'Name':self.name, 'Score':self.score_culculate()}).content)
    ...
    def event_check(self, event):
        ...
        if self.input_field.final_text != '':
            ...
            if not self.requst_timer.active:
                requests.post(url=f'http://{self.url}/session', json={'Name':self.name})
        else:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                requests.post(f'http://{self.url}/post_permission', json={'Name':self.name})
    ...
    def run(self):
        ...
        while not done:
            if not self.requst_timer.active:
                self.battle_permissions = list(get_info(f'http://{self.url}/get_permissions').values())
                self.players_scores = list(get_info(f'http://{self.url}/get_scores').values())
                self.requst_timer.start()
            if len(self.players_scores) == self.players_count and self.table.content == []:
                self.table.content = list(get_info(f'http://{self.url}/score_sort').values())
            # обновление таймеров
            ...
            self.draw()
            if self.in_menu:
                self.screen.blit(pygame.image.load('./assets/images/very_cool_name_for_png_file.png'), (0,0))
